...
class HackerRank
  ...
  def self.candies(n, ratings)
    ...
    i = 1
    while i < n do
      given_candies[i] = given_candies[i - 1] + 1 if ratings[i] > ratings[i - 1]
      i += 1
    end
  ...
  def self.unbounded_knapsack_sum_r(array, max_amount, current_amount = 0, hashed_results = {})
    sums = []
    ...
    array.each do |e|
      if current_amount + e <= max_amount
        ...
        if result
          ...
          sum = unbounded_knapsack_sum_r(array, max_amount, current_amount + e, hashed_results)
  ...
  def self.unbounded_knapsack_sum(array, max_amount)
  ...
  def self.red_john_is_back(n, primes = generate_primes)
    dp = Array.new(41) { 0 }
    dp[0] = 1
    dp[1] = 1
    dp[2] = 1
  ...
  def self.lpermutation(string, nth)
    size = string.length
    facts = factsupto(size)
    permutation = Array.new(size)
    nth -= 1
    j = 1
